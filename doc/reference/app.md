...
The `createRoot` method returns an object with `prepare`, `commit`, `mount`,
and `destroy` methods.
...
### Two-phase mounting: `prepare` and `commit`

`mount(target)` is the common case and does everything in one step: start
the render phase, wait for `onWillStart` callbacks to settle, then mount
into the target. For more control, the root exposes these as two distinct
phases:
...
- **`prepare()`** starts the render phase without a DOM target.
  ...
  elements exist yet.
- **`commit(target, options?)`** mounts the prepared bdom into `target`
  and fires `onMounted` hooks. Calling `commit` without a prior `prepare`
  prepares implicitly. Returns the same promise as `mount` (resolves with
  the component instance).
- **`mount(target, options?)`** is equivalent to `commit(target, options)`.
...
- **Off-screen rendering**: build the DOM in memory, inspect or
  pre-measure it, then commit later.
- **Parallel loading**: call `prepare()` on multiple roots at once and
  wait on `Promise.all([...])` before committing them.
...
await ready;
await root.commit(targetElement);
```

If you call `prepare()` but never `commit()`, the root never mounts
(and its mount promise stays pending). `destroy()` still cleans up the
prepared subtree in either case.

`validateTarget` runs at `commit` time, not at `prepare`, so a bad
