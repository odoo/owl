...
describe("does not observe", () => {
  ...
  test("a computation validating a reactive set does not subscribe to it", () => {
  ...
  test("a computation validating a reactive map does not subscribe to it", () => {
  ...
  test("an issue reports the raw value", () => {
    ...
    const map = proxy(new Map([["k", object]]));
    expect(validateType(map, t.map(t.string(), t.string()))[0].received).toBe(object);
  });
