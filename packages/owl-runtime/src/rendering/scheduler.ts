...
export class Scheduler {
  ...
  processTasks() {
    ...
    // resume after this tick — they then call flush() to schedule the next
    // tick for their commit.
    //
    ...
    // scheduler itself to resume — _schedule() posts to the same
    // MessageChannel as a normal tick, so the browser can paint, process
    // input, and run scroll/IntersectionObserver callbacks between commit
    // batches.
