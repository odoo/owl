...
`capture(ctx)` mechanism only captured a frozen copy of the *values*, not the
actual *bindings*. The same issue existed with `captureExpression`. This meant
...
- **Loops create a new scope per iteration** (`let` semantics). Each iteration
  ...
  event handlers, `t-call` bodies) correctly close over the *bindings* for that
