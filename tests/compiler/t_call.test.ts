...
describe("t-call (template calling)", () => {
  ...
  test("values set in body are in call context", () => {
    ...
    const main = `<span><t t-call="sub" foo="'ok'"><t t-set="foo" t-value="'ko'"/></t></span>`;
    context.addTemplate("sub", sub);
    context.addTemplate("main", main);
  ...
  test("does not inherit context", () => {
    const context = new TestContext();
    const sub = '<t t-out="foo"/><t t-out="bar"/>';
    const main = `<div><t t-set="foo" t-value="1"/><t t-set="bar" t-value="2"/><t t-call="sub" bar="bar"/></div>`;
    context.addTemplate("sub", sub);
    context.addTemplate("main", main);
  ...
  test("scoped parameters, part 2", () => {
    const context = new TestContext();
    const sub = '<t t-out="foo"/><t t-out="0"/>';
    const main = `
        <div>
  ...
  test("recursive template, part 2", () => {
    ...
    const Parent = `
        <div>
          <t t-call="nodeTemplate" node="root"/>
        </div>`;
    ...
    const nodeTemplate = `
        <div>
          ...
          <t t-foreach="node.children or []" t-as="subtree" t-key="subtree_index">
              <t t-call="nodeTemplate" node="subtree"/>
          </t>
        </div>`;
  ...
  test("recursive template, part 4: with t-set recursive index", () => {
    ...
    const Parent = `
        <div>
          <t t-call="nodeTemplate" recursive_idx="1" node="root"/>
        </div>`;
    ...
    const nodeTemplate = `
        <div>
          ...
          <t t-foreach="node.children or []" t-as="subtree" t-key="subtree_index">
            <t t-call="nodeTemplate" node="subtree" recursive_idx="recursive_idx"/>
          </t>
        </div>`;
  ...
  test("t-call with t-set inside and outside. 2", () => {
    ...
    const wrapper = `<p><t t-set="w" t-value="'fromwrapper'"/><t t-call="main" w="w" list="list"/></p>`;
  ...
  test("dynamic t-call", () => {
    ...
    const main = `<div><t t-call="{{template}}" val="val"/></div>`;
  ...
  test("t-call-context", () => {
    const context = new TestContext();
    context.addTemplate("sub", /* xml */ `<span><t t-out="value"/></span>`);
    context.addTemplate("main", /* xml */ `<t t-call="sub" t-call-context="obj"/>`);
  ...
  test("t-call with attributes and t-out='0'", () => {
    const context = new TestContext();
    context.addTemplate("sub", `<span><t t-out="v_2"/><t t-out="0"/></span>`);
    context.addTemplate("main", `<t t-call="sub" v_2="1">Hello</t>`);
    expect(context.renderToString("main")).toBe("<span>1Hello</span>");
  });

  test("t-call and translation contexts on parent template", () => {
    const translations: Record<string, Record<string, string>> = {
      fr: { game: "jeu" },
      pt: { title: "titulo" },
    };
    const translateFn = jest.fn((s: string, ctx: string) => translations[ctx]?.[s] ?? s);

    const context = new TestContext({ translateFn });
    context.addTemplate(
      "sub",
      /* xml */ `<span title="title"><t t-out="title"/><t t-out="0"/></span>`
    );
    context.addTemplate(
      "main",
      /* xml */ `
        <t t-set="title" t-translation-context="pt">title</t>
        <t t-call="sub" title="title" t-translation-context="fr">game</t>
      `
    );

    expect(context.renderToString("main")).toBe(/* xml */ `<span title="title">titulojeu</span>`);
  });

  test("t-call and translation contexts in both templates", () => {
    const translations: Record<string, Record<string, string>> = {
      fr: { game: "jeu" },
      pt: { title: "titulo" },
    };
    ...
    context.addTemplate(
      "sub",
      /* xml */ `
        <span t-translation-context-title="pt" title="title"><t t-out="title"/><t t-out="0"/>title</span>`
    );
    context.addTemplate(
      "main",
      /* xml */ `
        <t t-set="game" t-translation-context="fr">game</t>
        <t t-call="sub" title="'title'"><t t-out="game" /></t>
      `
    );

    expect(context.renderToString("main")).toBe(
      /* xml */ `<span title="titulo">titlejeutitle</span>`
    );
  });
});
