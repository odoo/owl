...
class Counter extends Component {
  static override template = xml`
    <span t-esc="state.value"/> <button class="counter-btn btn btn-sm btn-outline-secondary" t-on-click="increment">+</button>
  `;

  state!: { value: number };

  override setup() {
    this.state = useState({ value: this.props.initialValue as number });
  }
...
class Root extends Component {
  ...
  state!: { rows: Row[]; selectedId: number };

  override setup() {
    this.state = useState({ rows: [] as Row[], selectedId: 0 });
  }
